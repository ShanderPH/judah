...
- O owner configurado da IA é removido no handoff; owners humanos existentes
  são preservados.
...
- `HUBSPOT_SALOMAO_TICKET_OWNER_ID` deve ser o ID de owner de ticket, não o
  actor ID do Conversations.
- Se a variável ficar vazia, pipeline e status são atualizados, mas o owner
  atual é preservado.
