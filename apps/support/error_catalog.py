...
_ERRORS: Final[dict[str, CatalogedError]] = {
    ...
    "stale_ticket": CatalogedError(
        catalog_code="SUP-HUBSPOT-006",
        message="O ticket não pertence mais ao pipeline e estágio elegíveis para atribuição.",
        category="provider_precondition",
        retryable=False,
        action_taken="A reserva foi compensada e a linha foi colocada em quarentena sem alterar o proprietário.",
        operator_hint="Confirme pipeline e estágio atuais antes de reavaliar a linha.",
    ),
    "hubspot_manual_owner_observed": CatalogedError(
        catalog_code="SUP-HUBSPOT-007",
        message="Um proprietário externo já estava presente antes da alteração automática.",
        category="provider_convergence",
        retryable=False,
        action_taken="A reserva foi compensada e nenhuma alteração de proprietário foi enviada.",
        operator_hint="Preserve o proprietário atual e confirme a projeção manual pelo evento de owner.",
    ),
    "hubspot_precondition_unreadable": CatalogedError(
        catalog_code="SUP-HUBSPOT-008",
        message="Não foi possível validar pipeline, estágio e proprietário imediatamente antes da atribuição.",
        category="provider_precondition",
        retryable=True,
        action_taken="A reserva foi compensada e uma nova tentativa limitada foi agendada.",
        operator_hint="Verifique a leitura do ticket no HubSpot antes de liberar novos efeitos.",
    ),
